...
def _checkpoint_name(train_aug: bool, test_level: str) -> str:
    return f"vit_trainAug_{'on' if train_aug else 'off'}_test_{test_level}.pth"
...
def load_vit_summaries(saved_models_dir: str | Path) -> list[RunSummary]:
...
def plot_vit_augmentation_groups(saved_models_dir: str | Path = "saved_models"):
    summaries = load_vit_summaries(saved_models_dir)

    if not summaries:
        raise FileNotFoundError(f"No ViT checkpoints found in {saved_models_dir}")
